...
def apply_game_settings():
    """อัปเดตเสียงและค่าต่างๆ ตาม Settings Menu ให้ครอบคลุมทั้งเกม"""
    # 1. Music Volume
    pygame.mixer.music.set_volume(settings_menu.music_volume)
    
    # 2. SFX Volume (Global Sounds)
    vol = settings_menu.sfx_volume
    global_sfx = [
        SPAWN_SOUND, HIT_SOUND, PLAYER_HURT_SOUND, WHOOSH_SOUND, 
        CRYSTAL_BREAK_SOUND, IMPACT_THUD_SOUND, BOSS_ROAR_SOUND, ENDING_AMBIENT
    ]
    for sfx in global_sfx:
        if sfx: sfx.set_volume(vol)
    
    # 3. Player SFX
    if 'player' in globals() and player:
        player_sfx = [
            '_walk_sound', '_run_sound', '_jump_sound', '_land_sound', 
            '_heal_sound', '_attack_sound', '_block_sound', '_death_sound'
        ]
        for sfx_name in player_sfx:
            sfx_obj = getattr(player, sfx_name, None)
            if sfx_obj:
                # ปรับบาลานซ์เสียงบางอันให้เบาลงกว่าปกติ
                m_vol = vol
                if sfx_name in ('_walk_sound', '_run_sound'): m_vol *= 0.6
                sfx_obj.set_volume(m_vol)
    
    # 4. Boss & Monster SFX
    if 'current_boss' in globals() and current_boss:
        boss_sfx = ['_roar_sound', '_spawn_sound', '_death_sound', '_hurt_sound']
        for sfx_name in boss_sfx:
            sfx_obj = getattr(current_boss, sfx_name, None)
            if sfx_obj: sfx_obj.set_volume(vol * 0.8)

    for m in current_monsters:
        if hasattr(m, '_attack_sound') and m._attack_sound:
            m._attack_sound.set_volume(vol * 0.7)
        if hasattr(m, '_hurt_sound') and m._hurt_sound:
            m._hurt_sound.set_volume(vol * 0.5)
        if hasattr(m, '_spell_sound') and m._spell_sound:
            m._spell_sound.set_volume(vol * 0.6)
...
def reset_game():
    ...
    global death_cinematic_timer, slow_mo_timer, hit_stop_timer, ending_step, ending_timer, ending_fade_alpha, is_scene_fading, scene_fade_alpha, camera_scroll, boss_slomo_triggered
    current_scene = 1
    ...
    global old_scene, ambient_particles, particles, impact_particles, slash_fx_list, shockwaves, dust_particles, floating_texts
    old_scene = -1                   # บังคับให้ระบบมองว่าเปลี่ยนฉาก เพื่อเสกมอนฉาก 1
    
    # ล้างเอฟเฟกต์และอนุภาคที่ค้างอยู่
    ambient_particles[:] = []
    particles[:] = []
    impact_particles[:] = []
    slash_fx_list[:] = []
    ...
    # รีเซ็ตสถานะหน้าจอและเอฟเฟกต์ (Fix หน้าจอมืด/สโลว์หลัง Restart)
    death_cinematic_timer = 0
    slow_mo_timer = 0
    hit_stop_timer = 0
    ending_step = 0
...
while running:
    ...
    if game_state == "GAME":
        ...
        # วาดตัวเลข FPS ถ้าเปิดใช้งาน (ขวาบน ใต้ปุ่ม MENU)
        if settings_menu.show_fps:
            fps_text = f"FPS: {int(clock.get_fps())}"
            fps_font = pygame.font.SysFont("Courier New", 14, bold=True)
            fps_surf = fps_font.render(fps_text, False, (0, 255, 0))
            # วาดเงา
            screen.blit(fps_font.render(fps_text, False, (0, 0, 0)), (712, 52))
            screen.blit(fps_surf, (710, 50))
    ...
    if not player.is_alive and game_state == "GAME":
        ...
        if not dialogue_manager.is_showing:
            ...
            if death_cinematic_timer == 0:
                death_cinematic_timer = 120 # 2 วินาที
                slow_mo_timer = 60
