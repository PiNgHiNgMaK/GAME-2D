...
def change_scene(scene_num, player_new_x=None, player_new_right=None):
    global bg, game_areas, current_scene, health_potions, current_monsters, current_boss, current_boss_ui, scene_monster_data, scene_monsters_cleared
    ...
    current_monsters = []  # ล้างมอนสเตอร์ฉากเก่า (ป้องกันมอนตามไปฉากอื่น)
    # ล้างบอสถ้าออกจากฉาก 4 (ป้องกันบอส update/โจมตีผู้เล่นข้ามฉาก)
    if scene_num != 4:
        current_boss = None
        current_boss_ui = None
...
# ฟังก์ชันรีเซ็ตเกม
def reset_game():
    global current_scene, bg, game_areas, player, current_boss, current_boss_ui, game_state, game_over_menu, current_monsters, scene_monster_data, scene_monsters_cleared
    current_scene = 1
...
while running:
    ...
    if current_scene != old_scene and current_scene == 3:
        if current_scene in scene_monster_data:
            ...
            # ผู้เล่นมาครั้งแรก สร้าง Necromancer ใหม่
